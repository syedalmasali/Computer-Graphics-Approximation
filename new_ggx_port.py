...
def syed_bivar_fitting(elevation_h, elevation_d, ftemp):
    ...
    # brdf has samples in RGB, we want to take average across all 3 channels
    f_avg = f.mean(axis=-1)
    print("my f_avg", f_avg)
    print("my f_avg shape", f_avg.shape)
    '''
    '''
    ...
    #print("AHHHHHHH",recons_samples.shape)

    return recons_samples
    
    '''
    plt.imshow(recons_samples, origin='lower')
...
def plot_brdf_slice_hd_spherical():
    ...
    #Plot the MEAN RGB SUMMED error
    f_meanerror = np.sum((f.mean(axis=-1) - f_approx.mean(axis=-1))**2, axis=-1)
    print("mean", f_meanerror.shape)
    print("max mean error", f_meanerror.max())
    print()
    plt.figure()
    plt.title(f'total RGB mean error is {f_meanerror.sum()}')
    plt.plot(f_meanerror[:-4])
    ...
    print('max MS error at', i, j)
    print("max  MS error", f_mserror[i, j])
    print("max MS error", f_mserror.max())
    print()

    plt.figure()
    #plt.pcolormesh(f_error)
    plt.subplot(1, 2, 1)
    ...
    print('max error at', i, j)
    print("max error", f_error[i, j])
    print("max error", f_error.max())
    print()
